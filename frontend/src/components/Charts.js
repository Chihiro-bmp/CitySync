      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', pointerEvents:'none' }}>
        {hov ? (
          <>
            ...
            <div style={{ fontSize:11, color:t.textSub, marginTop:4, fontFamily:fonts.ui, textAlign:'center', maxWidth:80 }}>{hov.label}</div>
          </>
        ) : (
          <>
            <div style={{ fontSize:22, fontWeight:700, color:t.text, fontFamily:fonts.ui, letterSpacing:'-0.5px', lineHeight:1 }}>{label}</div>
            {sublabel && <div style={{ fontSize:11, color:t.textSub, marginTop:4, fontFamily:fonts.mono, textAlign:'center' }}>{sublabel}</div>}
          </>
        )}
      </div>
    </div>
...
export const LineChart = ({ lines, t, isDark, unit = 'units', height = 200 }) => {
  const [hovered, setHovered] = useState(null); // { lineIdx, pointIdx }
  ...
  const pathFor = (pts) => pts.map((p, i) => `${i === 0 ? 'M' : 'L'} ${xPos(i)} ${yPos(p.value)}`).join(' ');
    ...
    <div style={{ position:'relative' }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', height:'auto', overflow:'visible' }}>
        ...
        {hovered && (() => {
          ...
          const tx   = xPos(hovered.pointIdx);
          const ty   = yPos(pt.value) - 14;
          const boxW = 80;
          const bx   = Math.max(PAD.left, Math.min(tx - boxW/2, W - PAD.right - boxW));
          return (
            <g>
              <rect x={bx} y={ty - 26} width={boxW} height={28} rx={6}
                fill={isDark ? '#1A2235' : '#fff'}
                stroke={t.border} strokeWidth={1}
                style={{ filter:'drop-shadow(0 2px 8px rgba(0,0,0,0.15))' }} />
              <text x={bx + boxW/2} y={ty - 14} textAnchor="middle"
                fontSize={12} fontWeight="600" fill={line.color} fontFamily={fonts.ui}>
                {pt.value} {unit}
              </text>
              <text x={bx + boxW/2} y={ty - 2} textAnchor="middle"
...
// ─── BAR CHART (grouped) ─────────────────────────────────────────────────────
export const BarChart = ({ data, gradient, glow, unit, t, isDark }) => {
  const [hovered, setHovered] = useState(null);
      ...
      <div style={{ display:'flex', gap:0 }}>
        ...
        <div style={{ flex:1, position:'relative' }}>
            ...
            {data.map((d, i) => {
              ...
              return (
                <div key={i} style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', height:'100%', justifyContent:'flex-end', position:'relative' }}
                  ...
                  <div style={{ width:'100%', borderRadius:'5px 5px 0 0', height:`${Math.max(pct,2)}%`, background: isHov ? gradient : (isDark ? 'rgba(255,255,255,0.06)' : '#E8ECF5'), boxShadow: isHov ? `0 0 14px ${glow}` : 'none', transition:'all 0.2s', cursor:'pointer' }} />
                </div>
              );
            })}
          </div>
          <div style={{ display:'flex', gap:5, marginTop:6 }}>
            {data.map((d, i) => (
              <div key={i} style={{ flex:1, textAlign:'center', fontSize:10, color: hovered === i ? t.primary : t.textMuted, fontFamily:fonts.mono, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', transition:'color 0.15s' }}>
                {d.label}
              </div>
            ))}
          </div>
